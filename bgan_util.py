...
class CelebDataset():
    ...
    def next_batch(self, batch_size, class_id=None):
        got_batch = False
        while not got_batch:
            rand_idx = np.random.choice(list(range(self.num_train)), size=(2*batch_size,), replace=False)
    ...
    def test_batch(self, batch_size):
        got_batch = False
        while not got_batch:
            rand_idx = np.random.choice(list(range(self.num_train, self.num_train + self.num_test)),
...
class SVHN():
    ...
    def __init__(self, path, subsample=None):
        ...
        if subsample is not None:
            rand_idx = np.random.choice(list(range(self.imgs.shape[0])), 
                                        size=(int(self.imgs.shape[0]*subsample),), 
                                        replace=False)  
            self.imgs, self.labels = self.imgs[rand_idx], self.labels[rand_idx]

    def next_batch(self, batch_size, class_id=None):
        rand_idx = np.random.choice(list(range(self.imgs.shape[0])), size=(batch_size,), replace=False)    
        return self.imgs[rand_idx], self.labels[rand_idx]
    

    def test_batch(self, batch_size):
        rand_idx = np.random.choice(list(range(self.test_imgs.shape[0])),
